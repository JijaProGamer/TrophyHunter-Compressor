...
class MapperModel(nn.Module):
    def __init__(self, latent_size, small_map_size, map_size, num_classes):
        ...
        #self.resnet1 = nn.Conv2d(16, 16, kernel_size=3, padding=1)
        #self.bn_resnet1 = nn.BatchNorm2d(16)

        self.begin1 = nn.Conv2d(16, 16, kernel_size=3, padding=1)
        self.bn_b1 = nn.BatchNorm2d(16)
        self.begin2 = nn.Conv2d(16, 16, kernel_size=3, padding=1)
        self.bn_b2 = nn.BatchNorm2d(16)

        self.begin3 = nn.Conv2d(16, 16, kernel_size=3, padding=2)
        self.bn_b3 = nn.BatchNorm2d(16)
        
        self.up = nn.ConvTranspose2d(16, 16, kernel_size=3, stride=3, padding=1)
        self.bn_up = nn.BatchNorm2d(16)

        self.resnet2 = nn.Conv2d(16, 16, kernel_size=3, padding=1)
        self.bn_resnet2 = nn.BatchNorm2d(16)
    ...
    def forward(self, x):
        #start = self.bn_resnet1(self.resnet1(x))

        x = F.gelu(self.bn_b1(self.begin1(x)))
        x = F.gelu(self.bn_b2(self.begin2(x)))
        x = F.gelu(self.bn_b3(self.begin3(x)))
        ...
        start = self.bn_resnet2(self.resnet2(x))
