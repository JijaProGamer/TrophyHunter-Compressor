...
class Encoder(nn.Module):
    def __init__(self, args, input_resolution, latent_dims):
        ...
        self.res2 = ResNetLayer(in_channels=128, out_channels=256, scale='down', num_layers=1) # 2
        self.res3 = ResNetLayer(in_channels=256, out_channels=512, scale='down', num_layers=1) # 3
        self.res4 = ResNetLayer(in_channels=512, out_channels=1024, scale='down', num_layers=1) # 4

        self.simplify = DownscaleFilters(in_channels=1024, out_channels=256)
        self.simplify_2 = DownscaleFilters(in_channels=256, out_channels=64)
        self.simplify_3 = DownscaleFilters(in_channels=64, out_channels=16)
...
class Decoder(nn.Module):
    def __init__(self, args, input_resolution, latent_dims):
        ...
        self.simplify = DownscaleFilters(in_channels=16, out_channels=64)
        self.simplify_2 = DownscaleFilters(in_channels=64, out_channels=256)
        self.simplify_3 = DownscaleFilters(in_channels=256, out_channels=1024)

        self.res1 = ResNetLayer(in_channels=1024, out_channels=512, scale='up', num_layers=1) # 4 
        self.res2 = ResNetLayer(in_channels=512, out_channels=256, scale='up', num_layers=1) # 3
        self.res3 = ResNetLayer(in_channels=256, out_channels=128, scale='up', num_layers=1) # 2
